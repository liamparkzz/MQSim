...
- 기준 커밋: `7d881b9126bcc21f307a3ab8e18be76b292ed522`. 작업 브랜치 `work/issues-001-004`에서 작성 후 백업 `main`에 반영한다.
- 소스 6개 파일·12개 변경 블록: 002의 erase-count 차이 반환; 003의 LPA barrier 대기 요청 재제출, overfull 쓰기의 잠금/CMT 재확인, GC 데이터·매핑 이동 목적지 program 수 보호, GREEDY 및 최종 후보 안전 검사, 정적 WL 시작 주소 수정.
- 001 설정 전달·004 매핑 쓰기 스케줄링/의존성 해제는 현재 PR79 통합본을 유지. 설정·workload·테스트·동기화 코드는 변경하지 않았다.
- 확인: MSVC 전체 컴파일 및 링크 성공, 최종 GC 소스 재컴파일/재링크 성공. 시뮬레이터 실행·행동 검증 없음. 과거 재현의 잔류·중복 잠금 해소 여부는 사용자 검증 대기이며, 001~004 전체 통과로 표시하지 않는다.
- 설명: `docs/issues-001-004-followup.md`. 각 변경 블록에 파일/줄·diff·수정 이유·동작·검증 범위를 기록했다. Obsidian commit 이력에 동일 설명과 커밋 일시를 보존한다. Baseline은 불변, Current는 원격 백업 커밋과 일치시킨다.
- 빌드 기록: `C:/CODEX/MQsimExperiments/20260910-issues001-004-code`. 실행 중인 신규 시뮬레이션 없음. 다음 작업은 사용자 설계에 따른 검증.

## 2026-09-10 PR79 변경 코드별 해설 보완

- 사용자 요청에 따라 기존 Obsidian `Code/History/commit-817019625fd7442d18ec56882540dadb82324d81.md`를 한 페이지의 코드별 설명으로 보완했다. 원본 커밋 시각·본문과 고정 diff를 보존한다.
- 실제 반영 23개 파일(소스 22 + .gitignore)의 56개 변경 블록마다 수정 이유·동작 설명·검증 범위를 붙였다. 원본에 이미 반영돼 이번 diff에 없는 PR79 수정도 별도 설명한다.
- 앞으로도 exporter의 일괄 요약에 그치지 않고 변경 블록별 해설까지 완료하도록 양쪽 AGENTS.md에 명시했다. 이번 작업은 설명/지침 변경이며 시뮬레이터 src 변경 없음.

## 2026-09-10 자동 기록 재개 확인

- PR79 코드 `8170196`과 자동 생성 변경 이력이 모두 GitHub에 반영됐음을 확인했다. 원본 Baseline은 보존돼 있다.
- 노트북에서 예약 작업이 Queued에 머무는 조건 확인: 배터리 사용 중 실행 금지/중단 설정이 켜져 있었다. 설치 스크립트에 배터리 실행 허용 및 전원 전환 시 계속 실행을 추가한다. 예약 간격과 기존 Git 동기화 범위는 유지한다.

## 2026-09-09 PR79 반영 및 자동 변경 설명 검증

- 사용자 요청에 따라 PR79 head `f42c4b82`의 변경을 기존 원본 계열 코드에 3-way 병합. 최신 원본의 미기록 페이지 읽기 보호·캐시 빈 목록 해제 등 기존 수정은 보존했다.
- PR79의 단일 ONFI 채널 객체에 대한 `delete[]`는 실제 `new` 할당에 맞춰 `delete`로 보정했다. 최초 Obsidian Baseline은 유지한다.
- 검증: MSVC 전체 빌드, 8조건/16프로세스 실행 통과. 각 실행 Host 128/128 완료, 매핑 쓰기 15완료/0잔류, WL 객체 설정 일치, 관측 유무 결과 XML 동일. 테스트 전후 src 149개 해시 불변.
- 상세 변경 설명·검증 한계: `docs/pr79-integration.md`. 실행 로그: `C:/CODEX/MQsimExperiments/20260909-pr79-integration`. 실행 중인 신규 시뮬레이션 없음.
- 002 마모 차이 오류는 미수정. 003 해제 후 완료 통지는 반영했으나 기존 잔류·중복 잠금 문제까지 해결됐다고 판단하지 않는다. 긴 재현 입력 재검증은 이번에 수행하지 않음.

## 2026-09-09 원본 코드 Obsidian 게시 및 수정 추적

- 후속 사용자 명확화: GitHub 백업 main을 코드 기준으로 삼고 수정 일시·이유·코드 동작 설명을 Obsidian에 기록. 내보내기는 원격 해시 확인 후 백업 커밋에서 읽으며 미전송 로컬 변경은 제외. 커밋 본문에 수정 이유/변경 설명/검증/관련 이슈를 작성한다.

- 사용자 요청에 따라 공식 원본 `51f0f2d3`를 Code/Baseline에 보존하고 Current 전문, 변경 이력, 원본 대비 누적 diff를 생성한다. PR79 수정본은 사용하지 않는다. src 149개 파일 공식 원본과 차이 없음 확인.
- 노트북 scripts/export-obsidian-code.ps1을 기존 5분 동기화 작업에 연결. 수정 직후 -Reason으로 이유를 붙여 실행한다. 데스크탑 자동 작업은 별도 연결 필요.
- 시뮬레이션을 시작하거나 원본 src를 수정하지 않았다. 자동 생성 노트와 상태는 Vault Git으로 공유한다.
- 검증: 독립 임시 Git 저장소에서 원본 보존, 무변경 재실행, CRLF 무시, 수정 diff, 파일 추가·삭제·이름 변경, 커밋별 이력 통과. 전체 게시 157개(소스 149 + 빌드/설정/문서 8). 누적 코드 차이 없음.
