...
/* Parses the command line arguments. -i and -w are mandatory (SSD configuration file and workload
* definition file, respectively). -t is optional: when given, it overrides the File_Path of every
* trace-based I/O flow in the loaded workload definition, which lets a shell script drive many
* trace files through the same workload/config XML pair without editing the XML each time, e.g.:
*   for f in /traces/*.csv; do ./MQSim -i cfg.xml -w workload-msrc.xml -t "$f" -b; done
* -b/--batch unconditionally skips the final "press any key" pause (see the isatty() note at the
* end of main() for why this explicit flag exists in addition to automatic terminal detection).
* Arguments may appear in any order.*/
void command_line_args(int argc, char* argv[], string& input_file_path, string& workload_file_path, string& trace_file_path_override, bool& batch_mode)
{
	for (int arg_cntr = 1; arg_cntr < argc; arg_cntr++) {
		string arg = argv[arg_cntr];

		if (arg == "-i" && arg_cntr + 1 < argc) {
			input_file_path.assign(argv[++arg_cntr]);
		} else if (arg == "-w" && arg_cntr + 1 < argc) {
			workload_file_path.assign(argv[++arg_cntr]);
		} else if (arg == "-t" && arg_cntr + 1 < argc) {
			trace_file_path_override.assign(argv[++arg_cntr]);
...
//Extracts the file name (without directory or extension) of a path, accepting both '/' and '\' as
//directory separators so it works the same whether the path was given in Windows or Linux style.
string extract_file_stem(const string& path)
{
	size_t slash_pos = path.find_last_of("/\\");
	string file_name = (slash_pos == string::npos) ? path : path.substr(slash_pos + 1);
	size_t dot_pos = file_name.find_last_of(".");
	return (dot_pos == string::npos) ? file_name : file_name.substr(0, dot_pos);
}
...
void print_help()
{
	cout << "MQSim - SSD simulator with both NVMe and SATA host interface behavior, see ReadMe.md for details" << endl <<
		...
		"./MQSim -i path/to/config/file -w path/to/workload/file [-t path/to/trace/file] [-b]" << endl <<
		"  -i  path to the SSD configuration XML file (mandatory)" << endl <<
		"  -w  path to the workload definition XML file (mandatory)" << endl <<
		"  -t  path to a trace file that overrides the File_Path of every trace-based I/O flow" << endl <<
		"      in the workload definition (optional; lets a shell script iterate many trace" << endl <<
		"      files without editing the workload XML for each one)" << endl <<
		"  -b, --batch  never pause for a keypress at the end (recommended for shell-script loops)" << endl;
}

int main(int argc, char* argv[])
{
	string ssd_config_file_path, workload_defs_file_path, trace_file_path_override;
	bool batch_mode = false;
	command_line_args(argc, argv, ssd_config_file_path, workload_defs_file_path, trace_file_path_override, batch_mode);
	...
	//Apply the -t override (if given) to every trace-based flow in every I/O scenario
	string output_file_stem = extract_file_stem(workload_defs_file_path);
	if (!trace_file_path_override.empty()) {
		for (auto io_scen : *io_scenarios) {
			for (auto flow : *io_scen) {
				if (flow->Type == Flow_Type::TRACE) {
					((IO_Flow_Parameter_Set_Trace_Based*)flow)->File_Path = trace_file_path_override;
				}
			}
		}
		//Name the result/log files after the trace file rather than the (now shared/reused) workload
		//file, so that repeated invocations with different -t values do not overwrite each other's results
		output_file_stem = extract_file_stem(trace_file_path_override);
	}
	//Results and per-flow logs are written next to the workload definition file, as before, just
	//under output_file_stem instead of always the workload file's own name
	size_t workload_dir_end = workload_defs_file_path.find_last_of("/\\");
	string output_file_prefix = (workload_dir_end == string::npos ? "" : workload_defs_file_path.substr(0, workload_dir_end + 1)) + output_file_stem;
	...
	for (auto io_scen = io_scenarios->begin(); io_scen != io_scenarios->end(); io_scen++, cntr++) {
		...
		exec_params->Host_Configuration.Input_file_path = output_file_prefix;//Create Host_System based on the specified parameters
		...
		collect_results(ssd, host, (output_file_prefix + "_scenario_" + std::to_string(cntr) + ".xml").c_str());
	}
    cout << "Simulation complete." << endl;

	/* Only pause for a keypress when running interactively (e.g., double-clicked from Windows
	* Explorer). This is skipped when -b/--batch is given, or (as a convenience) when stdin isn't a
	* real terminal (redirected/piped input). isatty() is the standard, reliable signal for this on
	* native Linux terminals; pass -b explicitly in shell-script loops (e.g., a for-loop iterating
	* many trace files via -t) if you want a guarantee that never depends on how stdin happens to be
	* wired up in a particular shell/terminal-emulator combination.*/
	if (!batch_mode && isatty(fileno(stdin))) {
