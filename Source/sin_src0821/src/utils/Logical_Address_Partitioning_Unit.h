...
namespace Utils
{
	...
	public:
		static void Reset();
		/* Sets the multiplier of the host-visible address space. It is used for RAID (SSD-array)
		*  simulation: the geometry parameters describe ONE member SSD, but the host-visible logical
		*  space of the array is (SSD_no_in_array x member_SSD_space). Only the host-view functions
		*  (Start/End_lha_available_to_flow, LHA_count_allocate_to_flow_from_host_view, and
		*  Get_total_device_lha_count) are scaled; the device-view functions that size the address
		*  mapping domain of a member SSD remain unscaled.*/
		static void Set_address_space_multiplier(unsigned int multiplier);
	...
	private:
		...
		static unsigned int address_space_multiplier;
