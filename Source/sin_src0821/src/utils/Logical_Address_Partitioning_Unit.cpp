...
namespace Utils
{
	...
	unsigned int Logical_Address_Partitioning_Unit::address_space_multiplier = 1;

	void Logical_Address_Partitioning_Unit::Set_address_space_multiplier(unsigned int multiplier)
	{
		if (multiplier < 1) {
			PRINT_ERROR("Illegal address space multiplier value!")
		}
		address_space_multiplier = multiplier;
	}

	void Logical_Address_Partitioning_Unit::Reset()
	{
		initialized = false;
		address_space_multiplier = 1;
		pdas_per_flow.clear();
	...
	LHA_type Logical_Address_Partitioning_Unit::Start_lha_available_to_flow(stream_id_type stream_id)
	{
		if (initialized) {
			switch (hostinterface_type) {
				case HostInterface_Types::SATA:
					return start_lhas_per_flow[stream_id] * address_space_multiplier;
					break;
				case HostInterface_Types::NVME:
					return start_lhas_per_flow[stream_id] * address_space_multiplier;
					break;
	...
	LHA_type Logical_Address_Partitioning_Unit::End_lha_available_to_flow(stream_id_type stream_id)
	{
		if (initialized) {
			switch (hostinterface_type) {
				case HostInterface_Types::SATA:
					return (end_lhas_per_flow[stream_id] + 1) * address_space_multiplier - 1;
					break;
				case HostInterface_Types::NVME:
					return (end_lhas_per_flow[stream_id] + 1) * address_space_multiplier - 1;
					break;
	...
	LHA_type Logical_Address_Partitioning_Unit::LHA_count_allocate_to_flow_from_host_view(stream_id_type stream_id)
	{
		if (initialized) {
			switch (hostinterface_type) {
				case HostInterface_Types::SATA:
					return (end_lhas_per_flow[stream_id] - start_lhas_per_flow[stream_id] + 1) * address_space_multiplier;
					break;
				case HostInterface_Types::NVME:
					return (end_lhas_per_flow[stream_id] - start_lhas_per_flow[stream_id] + 1) * address_space_multiplier;
					break;
	...
	{
		return total_lha_no * address_space_multiplier;
	}
}
