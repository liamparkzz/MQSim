...
namespace SSD_Components
{
	...
	void Data_Cache_Manager_Base::Setup_triggers()
	{
		...
		/* The member SSDs of a RAID array do not have their own host interface; their cache managers
		*  receive requests directly from the array controller through Receive_user_request.*/
		if (host_interface != NULL) {
			Data_Cache_Manager_Base* dcm = this;
			host_interface->Connect_to_user_request_arrived_signal([dcm](User_Request* user_request) { dcm->process_new_user_request(user_request); });
		}
	}
	...
	void Data_Cache_Manager_Base::Receive_user_request(User_Request* user_request)
	{
