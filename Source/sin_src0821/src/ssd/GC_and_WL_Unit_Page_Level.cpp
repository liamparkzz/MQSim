	...
	void GC_and_WL_Unit_Page_Level::Check_gc_required(const unsigned int free_block_pool_size, const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
		if (free_block_pool_size < block_pool_gc_threshold) {
			...
			switch (block_selection_policy) {
				...
				{
					//The comparison baseline must itself be a validated (fully-written, non-write-frontier,
					//no ongoing program) candidate -- unconditionally starting from block 0 without checking
					//it lets block 0 win by default (and be selected for GC/erase) whenever no other block's
					//Invalid_page_count exceeds it, even while block 0 is still the active write frontier.
