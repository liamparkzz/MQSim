...
namespace SSD_Components
{
	...
	Flash_Block_Manager_Base::Flash_Block_Manager_Base(const std::string& id, GC_and_WL_Unit_Base* gc_and_wl_unit, unsigned int max_allowed_block_erase_count, unsigned int total_concurrent_streams_no,
		...
		id(id), overprovisioning_ratio(overprovisioning_ratio), end_of_life_threshold(end_of_life_threshold), bad_block_count(0), eol_reached(false)
	{
		total_block_count = channel_count * chip_no_per_channel * die_no_per_chip * plane_no_per_die * block_no_per_plane;
		original_op_block_budget = (unsigned int)(total_block_count * overprovisioning_ratio);
	...
	{
		double remaining_op_blocks = (double)original_op_block_budget - (double)bad_block_count;
		if (remaining_op_blocks < 0) {
			remaining_op_blocks = 0;
		}
		return remaining_op_blocks / (double)total_block_count;
	}
	...
	bool Flash_Block_Manager_Base::Retire_block_if_worn_out(Block_Pool_Slot_Type* block)
	{
		...
		block->Is_bad = true;
		bad_block_count++;
		if (!eol_reached && Get_current_op_ratio() <= end_of_life_threshold) {
			eol_reached = true;
			Device_Lifecycle_Monitor::Report_end_of_life(id, Get_current_op_ratio(), bad_block_count);
		}
		return true;
	}
