...
namespace SSD_Components
{
	...
	{
		//The histogram must have one bucket per possible erase count from 0 up to and INCLUDING
		//max_allowed_block_erase_count itself (a block that has just reached the configured PE-cycle
		//limit and is being retired as a bad block still needs a valid bucket to be recorded in).
		unsigned int histogram_size = max_allowed_block_erase_count + 1;
			...
			for (unsigned int chip_cntr = 0; chip_cntr < chip_no_per_channel; chip_cntr++) {
				...
				for (unsigned int die_cntr = 0; die_cntr < die_no_per_chip; die_cntr++) {
					...
					for (unsigned int plane_cntr = 0; plane_cntr < plane_no_per_die; plane_cntr++) {
						Block_erase_histogram[channel_cntr][chip_cntr][die_cntr][plane_cntr] = new unsigned int[histogram_size];
