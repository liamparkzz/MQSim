...
#define MAKE_TABLE_INDEX(LPN,STREAM)
	...
	* its own share of CMT (equal partitioning of CMT space among concurrent streams).*/
	...
	public:
		...
		void Invalidate_lpa_mapping(const stream_id_type stream_id, const LPA_type lpa);
		bool Is_lpa_locked_for_gc(const stream_id_type stream_id, const LPA_type lpa);
	private:
