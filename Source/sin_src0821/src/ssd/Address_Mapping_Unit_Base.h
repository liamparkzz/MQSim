...
namespace SSD_Components
{
	...
	public:
		...
		**********************************************************************************************************************/
		...
		/* Invalidates the current mapping (if any) of an LPA without writing new data anywhere: the LPA's
		* current physical page (if it has one) is marked invalid in the block bookkeeping, and the LPA's
		* mapping table entry is cleared (as if the LPA had never been written). This is used by the SWANS
		* RAID-0 array controller after migrating a zone's data to another member SSD: the destination
		* member gets a normal write (which creates its own mapping), but the source member's old page for
		* that (member-local) LPA is never touched by any ordinary write, so it must be explicitly
		* invalidated here -- otherwise it would remain "valid" forever from this member's own FTL
		* perspective while SWANS's array-level bookkeeping believes that physical zone is free for reuse,
		* which leads to an inconsistency the next time this member's GC tries to reclaim that block. */
		virtual void Invalidate_lpa_mapping(const stream_id_type stream_id, const LPA_type lpa) = 0;

		/* Public visibility of the GC lock check, so that the SWANS RAID-0 array controller can avoid
		* migrating (and then invalidating, see Invalidate_lpa_mapping above) an LPA that this member
		* SSD's own internal GC/wear-leveling is concurrently moving for its own, independent reasons.
		* Without this check, SWANS's migration and the member's own GC could race on the same LPA:
		* SWANS could invalidate the very page GC just relocated it to, corrupting the mapping table.*/
		virtual bool Is_lpa_locked_for_gc(const stream_id_type stream_id, const LPA_type lpa) = 0;
	protected:
		FTL* ftl;
