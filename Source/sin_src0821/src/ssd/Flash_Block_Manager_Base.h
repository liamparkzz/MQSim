...
#define All_VALID_PAGE 0x0000000000000000ULL
	...
	*/
	...
	public:
		...
		bool Is_bad = false;//Set once the block's erase count reaches the configured PE-cycle limit; a bad block is permanently retired and never returned to the free block pool
		void Erase();
	};
	...
	public:
		Flash_Block_Manager_Base(const std::string& id, GC_and_WL_Unit_Base* gc_and_wl_unit, unsigned int max_allowed_block_erase_count, unsigned int total_concurrent_streams_no,
			unsigned int channel_count, unsigned int chip_no_per_channel, unsigned int die_no_per_chip, unsigned int plane_no_per_die,
			unsigned int block_no_per_plane, unsigned int page_no_per_block, double overprovisioning_ratio, double end_of_life_threshold);
		...
		/* Bad-block / end-of-life management: a block that reaches max_allowed_block_erase_count
		* (Block_PE_Cycles_Limit) is permanently retired instead of being returned to the free block
		* pool -- exactly as a real SSD replaces a failed block with a spare block drawn from its
		* over-provisioning (OP) area. As more blocks are retired, the effective OP ratio (relative to
		* the device's original total block count) shrinks; once it drops to or below
		* end_of_life_threshold, the device is considered to have reached its end of life.*/
		bool Retire_block_if_worn_out(Block_Pool_Slot_Type* block);//Called right after a block finishes erasing; returns true (and permanently marks the block bad) if its erase count has reached the configured limit
		unsigned int Get_bad_block_count() const { return bad_block_count; }
		double Get_current_op_ratio() const;//(original OP block budget - bad_block_count) / total_block_count; starts at overprovisioning_ratio and decreases as blocks are retired
		bool Has_reached_end_of_life() const { return eol_reached; }
	protected:
		...
		unsigned int total_block_count;//channel_count * chip_no_per_channel * die_no_per_chip * plane_no_per_die * block_no_per_plane
		unsigned int original_op_block_budget;//total_block_count * overprovisioning_ratio, computed once at construction time
		unsigned int bad_block_count;
		bool eol_reached;
	};
}
