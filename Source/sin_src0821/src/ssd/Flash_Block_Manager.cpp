...
{
	Flash_Block_Manager::Flash_Block_Manager(const std::string& id, GC_and_WL_Unit_Base* gc_and_wl_unit, unsigned int max_allowed_block_erase_count, unsigned int total_concurrent_streams_no,
		...
		: Flash_Block_Manager_Base(id, gc_and_wl_unit, max_allowed_block_erase_count, total_concurrent_streams_no, channel_count, chip_no_per_channel, die_no_per_chip,
			plane_no_per_die, block_no_per_plane, page_no_per_block, overprovisioning_ratio, end_of_life_threshold)
	{
	}
	...
	void Flash_Block_Manager::Add_erased_block_to_pool(const NVM::FlashMemory::Physical_Page_Address& block_address)
	{
		...
		unsigned int reclaimed_pages = block->Invalid_page_count;
		...
		if (Retire_block_if_worn_out(block)) {
			//This block has reached its configured PE-cycle limit (Block_PE_Cycles_Limit) and is
			//permanently retired (bad block) instead of being returned to the free block pool. Its
			//reclaimed capacity is removed from the plane's page accounting entirely -- exactly how a
			//spare/OP block is consumed to replace a failed block in a real SSD: the plane's *usable*
			//capacity permanently shrinks by one block.
