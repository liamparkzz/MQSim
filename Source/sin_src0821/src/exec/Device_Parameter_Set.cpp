...
double Device_Parameter_Set::End_of_Life_Threshold = 0.05;//SSD reaches end of life when the remaining OP ratio drops to/below this value
...
SWANS_Parameter_Set Device_Parameter_Set::SWANS_Parameters;
...
void Device_Parameter_Set::XML_serialize(Utils::XmlWriter& xmlwriter)
{
	...
	SWANS_Parameters.XML_serialize(xmlwriter);
	...
	try
	{
		for (auto param = node->first_node(); param; param = param->next_sibling()) {
			if (strcmp(param->name(), "Seed") == 0) {
				...
				if (End_of_Life_Threshold < 0 || End_of_Life_Threshold >= Overprovisioning_Ratio) {
					PRINT_MESSAGE("The specified End_of_Life_Threshold should be smaller than Overprovisioning_Ratio and non-negative; the SSD would otherwise reach end of life immediately (or never).")
				}
			...
			else if (strcmp(param->name(), "SWANS_Parameter_Set") == 0)
			{
				SWANS_Parameters.XML_deserialize(param);
			}
		}
	}
	catch (...)
