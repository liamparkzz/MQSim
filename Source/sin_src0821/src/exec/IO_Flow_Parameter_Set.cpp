...
void IO_Flow_Parameter_Set_Trace_Based::XML_serialize(Utils::XmlWriter& xmlwriter)
{
	...
	attr = "Relay_Mode";
	switch (Relay_Mode) {
		case Trace_Replay_Mode::FIXED_COUNT:
			val = "FIXED_COUNT";
			break;
		case Trace_Replay_Mode::UNTIL_END_OF_LIFE:
			val = "UNTIL_END_OF_LIFE";
			break;
		default:
			break;
	}
...
void IO_Flow_Parameter_Set_Trace_Based::XML_deserialize(rapidxml::xml_node<> *node)
{
	...
	try {
		for (auto param = node->first_node(); param; param = param->next_sibling()) {
			if (strcmp(param->name(), "Relay_Count") == 0) {
			...
			} else if (strcmp(param->name(), "Relay_Mode") == 0) {
				std::string val = param->value();
				std::transform(val.begin(), val.end(), val.begin(), ::toupper);
				if (strcmp(val.c_str(), "FIXED_COUNT") == 0) {
					Relay_Mode = Trace_Replay_Mode::FIXED_COUNT;
				} else if (strcmp(val.c_str(), "UNTIL_END_OF_LIFE") == 0) {
					Relay_Mode = Trace_Replay_Mode::UNTIL_END_OF_LIFE;
				} else {
					PRINT_ERROR("Unknown Relay_Mode specified for the trace based flow! Supported values: FIXED_COUNT, UNTIL_END_OF_LIFE")
				}
				...
				std::string val = param->value();
				std::transform(val.begin(), val.end(), val.begin(), ::toupper);
				if (strcmp(val.c_str(), "MQSIM_ASCII") == 0 || strcmp(val.c_str(), "ASCII") == 0) {
					Trace_Format = Trace_Format_Type::MQSIM_ASCII;
				} else if (strcmp(val.c_str(), "MSRC_CSV") == 0 || strcmp(val.c_str(), "MSRC") == 0) {
					Trace_Format = Trace_Format_Type::MSRC_CSV;
				} else {
					PRINT_ERROR("Unknown trace format specified for the trace based flow! Supported values: MQSIM_ASCII, MSRC_CSV")
				}
