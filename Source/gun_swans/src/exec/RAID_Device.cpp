...
void RAID_Device::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter) // Host_interface와 각 SSD들의 결과를 XML로 저장
{
	...
	}
	{
		...
		int first_eol_ssd_id = -1;
		sim_time_type first_eol_time = 0;
		uint64_t first_eol_bad_block_count = 0;
		double first_eol_effective_op_ratio = 0.0;
		uint64_t total_block_erase_count_all_ssds = 0;
		unsigned int max_block_erase_count_all_ssds = 0;
		double minimum_current_effective_op_ratio = std::numeric_limits<double>::max();
		...
		for (unsigned int ssd_idx = 0; ssd_idx < ssds.size(); ssd_idx++) {
			...
			const sim_time_type ssd_op_exhaustion_time = ftl->BlockManager->Get_op_exhaustion_time();
			const double ssd_current_effective_op_ratio = ftl->BlockManager->Get_current_effective_op_ratio();
			...
			if (ssd_current_effective_op_ratio < minimum_current_effective_op_ratio) {
				minimum_current_effective_op_ratio = ssd_current_effective_op_ratio;
			}
			if (ssd_op_exhausted && (first_eol_ssd_id < 0 || ssd_op_exhaustion_time < first_eol_time)) {
				first_eol_ssd_id = static_cast<int>(ssd_idx);
				first_eol_time = ssd_op_exhaustion_time;
				first_eol_bad_block_count = ftl->BlockManager->Get_eol_bad_block_count();
				first_eol_effective_op_ratio = ftl->BlockManager->Get_eol_effective_op_ratio();
			}
			...
			total_block_erase_count_all_ssds += ssd_summary.Sum_erase_count;
			if (ssd_summary.Max_erase_count > max_block_erase_count_all_ssds) {
				max_block_erase_count_all_ssds = ssd_summary.Max_erase_count;
			}
			...
			xmlwriter.Write_attribute_string("OP_Exhaustion_Time", std::to_string(ssd_op_exhaustion_time));
			xmlwriter.Write_attribute_string("Current_Effective_OP_Ratio", std::to_string(ssd_current_effective_op_ratio));
			xmlwriter.Write_attribute_string("EOL_Triggered", ssd_op_exhausted ? "true" : "false");
			xmlwriter.Write_attribute_string("EOL_Time", std::to_string(ssd_op_exhaustion_time));
			xmlwriter.Write_attribute_string("EOL_Bad_Block_Count", std::to_string(ftl->BlockManager->Get_eol_bad_block_count()));
			xmlwriter.Write_attribute_string("EOL_Effective_OP_Ratio", std::to_string(ftl->BlockManager->Get_eol_effective_op_ratio()));
		...
		std::string eol_tag = wl_tag + ".EOLSummary";
		xmlwriter.Write_open_tag(eol_tag);
		xmlwriter.Write_attribute_string("EOL_Triggered", any_op_exhausted ? "true" : "false");
		xmlwriter.Write_attribute_string("First_EOL_SSD_ID", std::to_string(first_eol_ssd_id));
		xmlwriter.Write_attribute_string("EOL_Time", std::to_string(first_eol_time));
		xmlwriter.Write_attribute_string("EOL_Bad_Block_Count", std::to_string(first_eol_bad_block_count));
		xmlwriter.Write_attribute_string("EOL_Effective_OP_Ratio", std::to_string(first_eol_effective_op_ratio));
		xmlwriter.Write_attribute_string("Bad_Block_Count", std::to_string(total_bad_blocks_all_ssds));
		xmlwriter.Write_attribute_string("Current_Effective_OP_Ratio",
			std::to_string(minimum_current_effective_op_ratio == std::numeric_limits<double>::max()
				? 0.0 : minimum_current_effective_op_ratio));
		xmlwriter.Write_attribute_string("Max_Block_Erase_Count", std::to_string(max_block_erase_count_all_ssds));
		xmlwriter.Write_attribute_string("Total_Block_Erase_Count", std::to_string(total_block_erase_count_all_ssds));
		xmlwriter.Write_close_tag();
