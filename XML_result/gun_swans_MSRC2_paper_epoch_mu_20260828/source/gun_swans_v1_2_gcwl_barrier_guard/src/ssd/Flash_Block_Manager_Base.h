...
#define All_VALID_PAGE 0x0000000000000000ULL
	...
	*/
	...
	public:
		...
		uint64_t Get_pending_retirement_count() const;
		uint64_t Get_op_consumed_block_count() const { return bad_block_count + Get_pending_retirement_count(); }
		uint64_t Get_op_spare_block_limit() const { return op_spare_block_limit; }
		uint64_t Get_total_block_count() const;
		double Get_current_effective_op_ratio() const;
		bool Is_op_exhausted() const { return op_exhausted; }
		sim_time_type Get_op_exhaustion_time() const { return op_exhaustion_time; }
		uint64_t Get_eol_bad_block_count() const { return eol_bad_block_count; }
		double Get_eol_effective_op_ratio() const { return eol_effective_op_ratio; }
	protected:
		...
		uint64_t op_spare_block_limit;
		bool op_exhausted;
		sim_time_type op_exhaustion_time;
		uint64_t eol_bad_block_count;
		double eol_effective_op_ratio;
