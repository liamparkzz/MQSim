...
namespace SSD_Components
{
	...
	Flash_Block_Manager_Base::Flash_Block_Manager_Base(GC_and_WL_Unit_Base* gc_and_wl_unit, unsigned int max_allowed_block_erase_count, unsigned int total_concurrent_streams_no,
		...
		block_no_per_plane(block_no_per_plane), pages_no_per_block(page_no_per_block), bad_block_count(0),
		op_spare_block_limit(0), op_exhausted(false), op_exhaustion_time(0),
		eol_bad_block_count(0), eol_effective_op_ratio(0.0)
	{
		const uint64_t total_blocks = static_cast<uint64_t>(channel_count) * chip_no_per_channel
			* die_no_per_chip * plane_no_per_die * block_no_per_plane;
		if (overprovisioning_ratio > 0.0 && total_blocks > 0) {
			op_spare_block_limit = std::max<uint64_t>(1,
				static_cast<uint64_t>(std::ceil(static_cast<double>(total_blocks) * overprovisioning_ratio)));
		}
	...
	uint64_t Flash_Block_Manager_Base::Get_total_block_count() const
	{
		return static_cast<uint64_t>(channel_count) * chip_no_per_channel
			* die_no_per_chip * plane_no_per_die * block_no_per_plane;
	}

	double Flash_Block_Manager_Base::Get_current_effective_op_ratio() const
	{
		const uint64_t total_blocks = Get_total_block_count();
		if (total_blocks == 0) {
			return 0.0;
		}
		const uint64_t consumed_blocks = Get_op_consumed_block_count();
		const uint64_t remaining_op_blocks = op_spare_block_limit > consumed_blocks
			? op_spare_block_limit - consumed_blocks : 0;
		return static_cast<double>(remaining_op_blocks) / static_cast<double>(total_blocks);
	}
	...
	unsigned int Flash_Block_Manager_Base::Get_min_max_erase_difference(const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
		...
		unsigned int min_erase = 0;
		unsigned int max_erase = 0;
		bool found = false;
		for (unsigned int i = 0; i < block_no_per_plane; i++) {
			if (plane_record->Blocks[i].Is_bad) continue;
			if (!found) {
				min_erase = max_erase = plane_record->Blocks[i].Erase_count;
				found = true;
			} else {
				min_erase = std::min(min_erase, plane_record->Blocks[i].Erase_count);
				max_erase = std::max(max_erase, plane_record->Blocks[i].Erase_count);
			}
		}
		return found ? max_erase - min_erase : 0;
	}
	...
	{
		unsigned int min_erased_block = block_no_per_plane;
		PlaneBookKeepingType *plane_record = &plane_manager[plane_address.ChannelID][plane_address.ChipID][plane_address.DieID][plane_address.PlaneID];

		for (unsigned int i = 0; i < block_no_per_plane; i++) {
			if (plane_record->Blocks[i].Is_bad) continue;
			if (min_erased_block == block_no_per_plane
				|| plane_record->Blocks[i].Erase_count < plane_record->Blocks[min_erased_block].Erase_count) {
				min_erased_block = i;
			}
		}
	...
	uint64_t Flash_Block_Manager_Base::Get_pending_retirement_count() const
	{
		uint64_t pending = 0;
		for (unsigned int channel_id = 0; channel_id < channel_count; channel_id++) {
			for (unsigned int chip_id = 0; chip_id < chip_no_per_channel; chip_id++) {
				for (unsigned int die_id = 0; die_id < die_no_per_chip; die_id++) {
					for (unsigned int plane_id = 0; plane_id < plane_no_per_die; plane_id++) {
						const PlaneBookKeepingType& plane = plane_manager[channel_id][chip_id][die_id][plane_id];
						for (unsigned int block_id = 0; block_id < block_no_per_plane; block_id++) {
							const Block_Pool_Slot_Type& block = plane.Blocks[block_id];
							if (!block.Is_bad && block.Has_ongoing_gc_wl
								&& block.Erase_count + 1 >= max_allowed_block_erase_count) {
								pending++;
							}
						}
					}
