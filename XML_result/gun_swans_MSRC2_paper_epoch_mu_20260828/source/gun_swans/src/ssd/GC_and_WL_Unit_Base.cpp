...
namespace SSD_Components
{
	...
	void GC_and_WL_Unit_Base::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction)
	{
		...
		switch (transaction->Type) {
			...
			case Transaction_Type::ERASE:
				...
				if (block_manager->Is_op_exhausted()) {
					return;
				}
				if (check_static_wl_required(transaction->Address)) {
					run_static_wearleveling(transaction->Address);
				}
				address_mapping_unit->Start_servicing_writes_for_overfull_plane(transaction->Address);//Must be inovked after above statements since it may lead to flash page consumption for waiting program transactions
	...
	{
		if (plane_record == nullptr || gc_wl_candidate_block_id >= block_no_per_plane
	...
	void GC_and_WL_Unit_Base::run_static_wearleveling(const NVM::FlashMemory::Physical_Page_Address plane_address)
	{
		...
		if (address_mapping_unit->Physical_block_has_locked_pages(wl_candidate_address)) {
			return;
		}

		//Run the state machine to protect against race condition
		// Mark the exact SSD/channel/chip/die/plane/block selected above.  Passing
		// only the block id implicitly constructed an address with block_id as the
		// channel id, leaving the real candidate unprotected and allowing a second
		// GC/WL operation to lock the same LPA.
