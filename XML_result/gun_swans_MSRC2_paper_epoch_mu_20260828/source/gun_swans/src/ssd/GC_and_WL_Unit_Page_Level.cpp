...
namespace SSD_Components
{
	...
	void GC_and_WL_Unit_Page_Level::Check_gc_required(const unsigned int free_block_pool_size, const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
		if (free_block_pool_size < block_pool_gc_threshold) {
			...
			if (gc_candidate_block_id >= block_no_per_plane
				|| pbke->Blocks[gc_candidate_block_id].Is_bad
			...
			if (address_mapping_unit->Physical_block_has_locked_pages(gc_candidate_address)) {
				// Returning here can deadlock a plane at the hard GC threshold: host
				// writes are already stopped, so nothing is guaranteed to call
				// Check_gc_required again after the overlapping LPA/MVPN barrier is
				// released.  Preserve the configured RGA choice when it is usable,
				// otherwise fall back to the best currently unlocked full block.
				flash_block_ID_type fallback_block_id = block_no_per_plane;
				NVM::FlashMemory::Physical_Page_Address fallback_address(plane_address);
				for (flash_block_ID_type block_id = 0; block_id < block_no_per_plane; block_id++) {
					if (pbke->Ongoing_erase_operations.find(block_id) != pbke->Ongoing_erase_operations.end()
						|| pbke->Blocks[block_id].Current_page_write_index != pages_no_per_block
						|| pbke->Blocks[block_id].Invalid_page_count == 0
					...
					fallback_address.BlockID = block_id;
					if (address_mapping_unit->Physical_block_has_locked_pages(fallback_address)) {
						continue;
					}
					if (fallback_block_id == block_no_per_plane
						|| pbke->Blocks[block_id].Invalid_page_count > pbke->Blocks[fallback_block_id].Invalid_page_count) {
				...
				gc_candidate_block_id = fallback_block_id;
				gc_candidate_address.BlockID = gc_candidate_block_id;
				block = &pbke->Blocks[gc_candidate_block_id];
			}
