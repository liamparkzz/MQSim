...
private:
	...
	struct Swans_Epoch_Record
	{
		uint64_t Sequence = 0;
		sim_time_type Time_ns = 0;
		double Mu_pp = 0.0;
		RAID_Policy::PolicyState State = RAID_Policy::PolicyState::NORMAL;
		int Hot_ssd = -1;
		int Cold_ssd = -1;
		bool Redirect_valid = false;
		uint64_t Migration_candidate_count = 0;
		uint64_t Total_host_write_sectors = 0;
		uint64_t Total_migration_write_sectors = 0;
		uint64_t Total_cumulative_write_sectors = 0;
		std::vector<uint64_t> Host_write_sectors;
		std::vector<uint64_t> Migration_write_sectors;
		std::vector<uint64_t> Cumulative_write_sectors;
	};
	...
	std::vector<Swans_Epoch_Record> swans_epoch_history;
