...
namespace SSD_Components
{
	...
	*/
	...
	bool Address_Mapping_Unit_Page_Level::Physical_block_has_locked_pages(const NVM::FlashMemory::Physical_Page_Address& block_address)
	{
		Block_Pool_Slot_Type* block = &(block_manager->plane_manager[block_address.ChannelID][block_address.ChipID][block_address.DieID][block_address.PlaneID].Blocks[block_address.BlockID]);
		NVM::FlashMemory::Physical_Page_Address addr(block_address);
		for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++) {
			...
			addr.PageID = pageID;
			if (block->Holds_mapping_data) {
				const MVPN_type mvpn = flash_controller->Get_metadata(addr.ChannelID, addr.ChipID, addr.DieID, addr.PlaneID, addr.BlockID, addr.PageID);
				if (is_mvpn_locked_for_gc(block->Stream_id, mvpn)) {
					return true;
				}
			} else {
				const LPA_type lpa = flash_controller->Get_metadata(addr.ChannelID, addr.ChipID, addr.DieID, addr.PlaneID, addr.BlockID, addr.PageID);
				if (is_lpa_locked_for_gc(block->Stream_id, lpa)) {
					return true;
				}
			}
		}
